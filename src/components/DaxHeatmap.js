...
export class DaxHeatmap extends HTMLElement {
	constructor() {
		super();
	...
	connectedCallback() {
		this.shadowRoot.innerHTML = `
			...
			<div class="heatmap-header">
				<div class="heatmap-title">DAX 40 Heatmap</div>
				<div class="stock-count" id="stock-count"></div>
			</div>
			<div class="heatmap-container" id="heatmap-container">
				<div class="loading">
					<div class="loading-spinner"></div>
					<div>Loading DAX data...</div>
				</div>
			</div>
		`;
	...
	async loadHeatmapData() {
		...
		try {
			console.log(`[DAX Heatmap] Fetching all 40 DAX stocks with optimized endpoint...`);
			...
			if (quoteData.quoteResponse && quoteData.quoteResponse.result) {
				quoteData.quoteResponse.result.forEach(quote => {
					if (quote && quote.regularMarketPrice && quote.regularMarketPrice > 0) {
						stockData.push({
							ticker: quote.symbol,
							name: quote.name || quote.symbol.replace('.DE', ''),
			...
			console.log(`[DAX Heatmap] Loaded ${stockData.length} stocks in ${loadTime}ms`);

			if (stockData.length === 0) {
				container.innerHTML = `
					<div class="error">
						<p>Unable to load DAX stock data for heatmap.</p>
			...
			console.error('Error loading DAX heatmap data:', error);
			container.innerHTML = `<div class="error">Error loading DAX heatmap data: ${error.message}</div>`;
		}
	}
	...
	renderHeatmap() {
		...
		sortedStocks.forEach(stock => {
			...
			// Remove .DE suffix for display
			tickerEl.textContent = stock.ticker.replace('.DE', '');
			...
			// Add click handler - pass the .DE symbol for Yahoo Finance compatibility
