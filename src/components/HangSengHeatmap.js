...
export class HangSengHeatmap extends HTMLElement {
	constructor() {
		super();
	...
	connectedCallback() {
		this.shadowRoot.innerHTML = `
			...
			<div class="heatmap-header">
				<div class="heatmap-title">Hang Seng Heatmap</div>
				<div class="stock-count" id="stock-count"></div>
			</div>
			<div class="heatmap-container" id="heatmap-container">
				<div class="loading">
					<div class="loading-spinner"></div>
					<div>Loading Hang Seng data...</div>
				</div>
			</div>
		`;
	...
	async loadHeatmapData() {
		...
		try {
			console.log(`[Hang Seng Heatmap] Fetching stocks with optimized endpoint...`);
			const startTime = performance.now();
			
			const response = await fetch(`${API_BASE_URL}/api/hangseng-heatmap`);
			
			if (!response.ok) {
				const errorText = await response.text();
				console.error(`[Hang Seng Heatmap] Backend error ${response.status}:`, errorText.substring(0, 200));
			...
			if (!contentType || !contentType.includes('application/json')) {
				...
				console.error('[Hang Seng Heatmap] Non-JSON response:', text.substring(0, 200));
			...
			if (quoteData.quoteResponse && quoteData.quoteResponse.result) {
				quoteData.quoteResponse.result.forEach(quote => {
					if (quote && quote.regularMarketPrice && quote.regularMarketPrice > 0) {
						stockData.push({
							ticker: quote.symbol,
							name: quote.name || quote.symbol.replace('.HK', ''),
			...
			console.log(`[Hang Seng Heatmap] Loaded ${stockData.length} stocks in ${loadTime}ms`);

			if (stockData.length === 0) {
				container.innerHTML = `
					<div class="error">
						<p>Unable to load Hang Seng stock data for heatmap.</p>
			...
			console.error('Error loading Hang Seng heatmap data:', error);
			container.innerHTML = `<div class="error">Error loading Hang Seng heatmap data: ${error.message}</div>`;
		}
	}
	...
	renderHeatmap() {
		...
		sortedStocks.forEach(stock => {
			...
			// Remove .HK suffix for display
			tickerEl.textContent = stock.ticker.replace('.HK', '');
			...
			// Add click handler - pass the .HK symbol for Yahoo Finance compatibility
